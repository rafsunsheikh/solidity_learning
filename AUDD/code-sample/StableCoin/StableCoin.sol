...
contract Common {
    ...
    mapping(TokenSupplyControlRequestType => uint256)
        internal tokenSupplyControlThresholds;
    /// mapping of all the token supply request.
    mapping(uint256 => TokenSupplyControlRequests)
        internal tokenSupplyControlRequests;

    /// mapping for the threshold count for Transaction Control.
    mapping(TransactionControlRequestType => uint256)
        internal transactionControlThresholds;
    /// mapping of all the Transaction control request.
    mapping(uint256 => TransactionControlRequests)
        internal transactionControlRequests;

    /// mapping for the threshold count for Signatory Control.
    mapping(SignatoryControlRequestType => uint256)
        internal signatoryControlThresholds;
    /// mapping of all the Signatory control request.
    mapping(uint256 => SignatoryControlRequests)
        internal signatoryControlRequests;

    /// mapping for the threshold count for Threshold Control.
    mapping(ThresholdControlRequestType => uint256)
        internal thresholdControlThresholds;
    /// mapping of all the Threshold control request.
    mapping(uint256 => ThresholdControlRequests)
        internal thresholdControlRequests;
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function getThresholds(
        ...
        for (uint256 i; i < requestTypeCount[reqType_]; i++) {
            if (reqType_ == RequestType.TOKEN_SUPPLY_CONTROL) {
                thresholds[i] = tokenSupplyControlThresholds[
                    TokenSupplyControlRequestType(i)
                ];
            } else if (reqType_ == RequestType.TRANSACTION_CONTROL) {
                thresholds[i] = transactionControlThresholds[
                    TransactionControlRequestType(i)
                ];
            } else if (reqType_ == RequestType.SIGNATORY_CONTROL) {
                thresholds[i] = signatoryControlThresholds[
                    SignatoryControlRequestType(i)
                ];
            } else {
                thresholds[i] = thresholdControlThresholds[
                    ThresholdControlRequestType(i)
                ];
            }
        }
        return thresholds;
    ...
    function getTokenSupplyControlRequest(
        ...
        require(
            tokenSupplyControlRequests[id_].owner != address(0),
            "INVALID_REQUEST!"
        );
    ...
    function getSignatoryControlRequest(
        ...
        require(
            signatoryControlRequests[id_].owner != address(0),
            "INVALID_REQUEST!"
        );
    ...
    function getThresholdControlRequest(
        ...
        require(
            thresholdControlRequests[id_].owner != address(0),
            "INVALID_REQUEST!"
        );
    ...
    function createThresholdControlRequest(
        ...
        require(
            thresholds_.length == requestTypeCount[reqType_],
        ...
        thresholdControlRequests[id_].subType = ThresholdControlRequestType
            .UPDATE;
    ...
    function updateThresholdControlRequest(
        ...
        require(
            thresholds_.length ==
                requestTypeCount[thresholdControlRequests[id_].reqType],
            "INVALID_THRESHOLD_COUNTS!"
        );
        for (uint256 i; i < thresholds_.length; i++) {
            require(thresholds_[i] > 0, "INVALID_THRESHOLD!");
    ...
    function vote(
        ...
        if (reqType_ == RequestType.TOKEN_SUPPLY_CONTROL) {
            ...
            bool isApproved = ArrayOps.isElement(
            ...
            tokenSupplyControlRequests[id_].status = tokenSupplyControlRequests[
                id_
            ].approvals.length >=
                tokenSupplyControlThresholds[
                    tokenSupplyControlRequests[id_].subType
                ]
                ? RequestStatus.ACCEPTED
                : RequestStatus.IN_PROGRESS;
        } else if (reqType_ == RequestType.TRANSACTION_CONTROL) {
            _isApprovable(
                transactionControlRequests[id_].owner,
                transactionControlRequests[id_].status
            );
            ...
            transactionControlRequests[id_].status = transactionControlRequests[
                id_
            ].approvals.length >=
                transactionControlThresholds[
                    transactionControlRequests[id_].subType
                ]
                ? RequestStatus.ACCEPTED
                : RequestStatus.IN_PROGRESS;
        } else if (reqType_ == RequestType.SIGNATORY_CONTROL) {
            _isApprovable(
                signatoryControlRequests[id_].owner,
                signatoryControlRequests[id_].status
            );
            ...
            if (approval_) {
                ...
                require(isApproved, "NOT_APPROVED!");
                signatoryControlRequests[id_].approvals = ArrayOps
                    .deleteFromArray(
                        signatoryControlRequests[id_].approvals,
                        msg.sender
                    );
            ...
            signatoryControlRequests[id_].status = signatoryControlRequests[id_]
                .approvals
                .length >=
                signatoryControlThresholds[
            ...
            _isApprovable(
                thresholdControlRequests[id_].owner,
                thresholdControlRequests[id_].status
            );
            ...
            if (approval_) {
                ...
                require(isApproved, "NOT_APPROVED!");
                thresholdControlRequests[id_].approvals = ArrayOps
                    .deleteFromArray(
                        thresholdControlRequests[id_].approvals,
                        msg.sender
                    );
            ...
            thresholdControlRequests[id_].status = thresholdControlRequests[id_]
                .approvals
                .length >=
                thresholdControlThresholds[
                    thresholdControlRequests[id_].subType
                ]
                ? RequestStatus.ACCEPTED
                : RequestStatus.IN_PROGRESS;
        } else {
        ...
        if (reqType_ == RequestType.TOKEN_SUPPLY_CONTROL) {
            ...
            if (
                ...
                if (
                    ...
                    tokenSupplyControlRequests[id_].wallet == address(this)
                ) {
                    _burn(
                        address(this),
                        tokenSupplyControlRequests[id_].amount
            ...
            for (
                uint256 i;
                i < signatoryControlRequests[id_].wallets.length;
                i++
            ) {
                if (
                    ...
                    require(
                        !isSignatory[signatoryControlRequests[id_].wallets[i]],
                        "EXISTING!"
                    );
                    isSignatory[
            ...
            for (
                uint256 i;
                i < requestTypeCount[thresholdControlRequests[id_].reqType];
                i++
            ) {
                if (
                    ...
                    tokenSupplyControlThresholds[
                        TokenSupplyControlRequestType(i)
                    ] = thresholdControlRequests[id_].thresholds[i];
                } else if (
                    thresholdControlRequests[id_].reqType ==
                    RequestType.TRANSACTION_CONTROL
                ) {
